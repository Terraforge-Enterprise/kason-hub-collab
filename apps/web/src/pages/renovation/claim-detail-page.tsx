// Admin page — Renovation Claim detail. Editor+ access (server enforces).
// Editors see metadata only; manager+ see full data + approve / reject /
// needs-amendment buttons. Reject and needs-amendment require a note (plain
// text) which is written to the transition row.
...
import {
  approveRenovationClaim,
  cancelRenovationClaim,
  deleteRenovationDocument,
  getRenovationClaim,
  getRenovationDocumentViewUrl,
  listRenovationClaimTransitions,
  needsAmendmentRenovationClaim,
  rejectRenovationClaim,
  type RenovationClaim,
  type RenovationClaimDocument,
} from "@/api/renovation-claims";
...
export default function RenovationClaimDetailPage() {
  ...
  // surfaced here so admins can see the full review history beyond the
  // single reviewerNote field on the claim.
  const transitions = useQuery({
    queryKey: ["renovation", "claims", id, "transitions"],
    queryFn: () => listRenovationClaimTransitions(id!),
    enabled: !!id,
  });

  const claim = useQuery({
    queryKey: ["renovation", "claims", id],
    queryFn: () => getRenovationClaim(id!),
    enabled: !!id,
  });

  const invalidate = () => {
    queryClient.invalidateQueries({ queryKey: ["renovation", "claims"] });
  };

  const approve = useMutation({
    mutationFn: () => approveRenovationClaim(id!),
    onSuccess: () => {
  ...
  const reject = useMutation({
    mutationFn: (n: string) => rejectRenovationClaim(id!, n),
    onSuccess: () => {
  ...
  const needsAmend = useMutation({
    mutationFn: (n: string) => needsAmendmentRenovationClaim(id!, n),
    onSuccess: () => {
  ...
  // Cancel — manager+ tidies up duplicates / mis-submitted claims.
  const cancel = useMutation({
    mutationFn: () => cancelRenovationClaim(id!),
    onSuccess: () => {
    ...
    <div className="space-y-6">
      ...
      {isEditor && (
        <Callout variant="info" title="Editor view">
          Package price, splits and document downloads are hidden. Approve and
          reject controls are limited to managers and admins.
        </Callout>
      )}

      {/* Reviewer note (if any prior decision) */}
      {c.reviewerNote && (
        <Callout
          variant={c.status === "rejected" ? "danger" : "warning"}
          title={
            c.status === "rejected" ? "Rejected" : "Reviewer note"
          }
        >
          {c.reviewerNote}
        </Callout>
        ...
        <CardContent>
          <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <DetailField
              label="Package ID"
              value={
                <span className="font-mono text-xs">{c.packageId}</span>
              }
            />
            <DetailField
              label="Payment type"
            ...
            {!isEditor && (
              <>
                ...
                />
                {c.paymentType === "offset_from_rental" && (
                  <DetailField
                    label="Monthly offset"
                    value={
                      c.monthlyOffsetAmount != null
                        ? `${formatRM(c.monthlyOffsetAmount)} / month`
                        : "—"
                    }
                  />
                )}
      ...
      {/* Splits */}
      {!isEditor && (
        <SplitsCard splits={c.splits} packagePrice={c.packagePrice} />
      )}

      {/* Documents */}
      {!isEditor && (
        <DocumentsCard claimId={c.id} documents={c.documents} />
      )}
      ...
      {/* Note dialog (reject + needs-amendment) */}
      <Dialog
      ...
      <CardContent className="space-y-2">
        {list.length === 0 ? (
          ...
          list.map((s) => {
            const computed =
              s.splitType === "percent"
                ? ((packagePrice ?? 0) * s.splitValue) / 100
                : s.splitValue;
            return (
              <div
                key={s.id}
...
function DocumentsCard({
  ...
  claimId: string;
  documents: RenovationClaim["documents"];
}) {
      ...
      <CardContent className="space-y-2">
        {docs.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No documents attached. Approval requires at least one quotation and
            one invoice.
          </p>
        ) : (
          docs.map((d) => (
            <DocumentRow key={d.id} claimId={claimId} doc={d} />
          ))
        )}
      </CardContent>
    </Card>
...
function DocumentRow({
  ...
  const open = async () => {
    setLoading(true);
    try {
      const { viewUrl } = await getRenovationDocumentViewUrl(claimId, doc.id);
  ...
  // Manager+ delete. Storage object is removed and the documents array on
  // the parent claim is shrunk in one transaction (server-side).
  const remove = useMutation({
    mutationFn: () => deleteRenovationDocument(claimId, doc.id),
    onSuccess: () => {
      toast.success("Document deleted");
      queryClient.invalidateQueries({ queryKey: ["renovation", "claims", claimId] });
    },
    onError: (err) => {
  ...
  return (
    <div className="flex items-center justify-between rounded border border-border/50 bg-background/40 px-3 py-2 text-sm">
      <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="text-xs text-muted-foreground capitalize">
            {doc.kind} · uploaded {fmtDateTime(doc.uploadedAt)}
          </div>
        </div>
      </div>
      <div className="flex items-center gap-1">
        <Button
          ...
          onClick={open}
          disabled={loading}
          title="Open in a new tab (signed URL, ~1 hour)"
        >
          {loading ? (
      ...
      <ConfirmAlert
        ...
        body={
          <>
            <strong>{doc.filename}</strong> will be removed from storage and the
            documents list. This cannot be undone.
          </>
        }
        confirmLabel="Delete"
        destructive
