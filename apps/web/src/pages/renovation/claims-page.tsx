// Admin page — Renovation Claims list. Editor+ access (server enforces).
// Editors see metadata-only rows; manager+ see prices and verb buttons in the
// detail page (this list never carries prices for editors because the API
// strips them).
import { useMemo, useState } from "react";
import { Link } from "react-router-dom";
import { useQuery } from "@tanstack/react-query";
import { Briefcase, Receipt, ClipboardList, ArrowUpRight } from "lucide-react";
...
import {
  listRenovationClaims,
  type RenovationClaim,
  type RenovationClaimStatus,
} from "@/api/renovation-claims";
import { statusBadgeVariant, statusLabel } from "./status-helpers";

const STATUS_OPTIONS: { value: RenovationClaimStatus | "all"; label: string }[] = [
  { value: "all", label: "All" },
...
export default function RenovationClaimsPage() {
  ...
  const [filterStatus, setFilterStatus] =
    useState<RenovationClaimStatus | "all">("all");
  ...
  const claims = useQuery({
    queryKey: ["renovation", "claims", apiFilters],
    queryFn: () => listRenovationClaims(apiFilters),
  ...
  if (claims.isError) {
    return (
      <Callout variant="danger" title="Failed to load claims">
        Unable to load renovation claims. Please refresh.
      </Callout>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Renovation cost claims linked to Sales Entries. Review packages,
          splits, and supporting documents.
        </p>
      </div>
      ...
      {isEditor && (
        <Callout variant="info" title="Editor view">
          Commission amounts, splits, and documents are hidden. Approve and
            ...
            <table className="w-full">
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr>
                    <td
                      colSpan={isEditor ? 6 : 7}
                      className="px-4 py-12 text-center text-sm text-muted-foreground"
                    >
                      No renovation claims match the current filters.
                    </td>
                  </tr>
                ) : (
                  rows.map((c) => (
    ...
    <tr className="border-b border-[var(--border)] transition hover:bg-[var(--page-bg)]">
      ...
      {!isEditor && (
        <td className="px-4 py-3.5 text-sm text-right font-medium">
          {claim.packagePrice != null ? formatRM(claim.packagePrice) : "—"}
        </td>
      )}
      <td className="px-4 py-3.5 text-sm">
...
function computeStats(rows: RenovationClaim[]) {
  let pending = 0;
  let approved = 0;
...
function toIsoStartOfDay(yyyyMmDd: string): string {
  // Anchor in UTC to avoid timezone drift on the server.
