...
import { portalResetPassword, portalVerifyResetToken } from "@/api/portal-auth";
...
export default function PortalResetPasswordPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await portalResetPassword(token!, pw1);
        ...
        {state.kind === "invalid" && (
          <div className="text-center" style={{ animation: "fadeIn 0.4s both" }}>
            ...
            <div className="mt-6 grid gap-3">
              <Link to="/portal/forgot-password" className="inline-flex min-h-12 w-full items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#B8963E_0%,#D4AF37_100%)] px-5 py-3 text-sm font-semibold text-white shadow-[0_24px_40px_-24px_rgba(180,150,62,0.50)]">Request a new link</Link>
        ...
        {(state.kind === "valid" || state.kind === "submitting") && (
          <>
            ...
            <form onSubmit={handleSubmit} className="grid gap-5">
              <PortalPasswordField label="New password" value={pw1} onChange={setPw1} show={show1} setShow={setShow1} hint="At least 8 characters" autoComplete="new-password" />
              <PortalPasswordField label="Confirm password" value={pw2} onChange={setPw2} show={show2} setShow={setShow2} autoComplete="new-password" />
...
function PortalPasswordField(props: {
  label: string;
  value: string;
