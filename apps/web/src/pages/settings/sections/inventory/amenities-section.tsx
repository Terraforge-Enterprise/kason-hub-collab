...
import type { Amenity } from "@/api/inventory-amenities";
...
export function AmenitiesSection({ canWrite }: { canWrite: boolean }) {
  const { data: rows = [], isLoading, isError } = useAmenities();
  ...
  function deactivate(r: Amenity) {
    update.mutate(
      { id: r.id, isActive: false },
      {
        onSuccess: () => {
          toast.success(`Amenity "${r.name}" deactivated.`);
  ...
  function handleDelete(target: Amenity) {
    remove.mutate(target.id, {
      onSuccess: ({ affectedUnitCount }) => {
        toast.success(
          `Amenity "${target.name}" deleted (${affectedUnitCount} unit${affectedUnitCount === 1 ? "" : "s"} updated).`,
        );
        setDeleteTarget(null);
      },
  ...
  if (isLoading) {
    return (
      <Surface title="Amenities" description="Loading...">
        <div className="p-6 text-sm text-muted-foreground">Loading amenity catalog…</div>
      </Surface>
    );
  }
  if (isError) {
    return (
      <Surface title="Amenities" description="Error">
        <div className="p-6 text-sm text-rose-500">Failed to load amenities. Refresh.</div>
      </Surface>
    );
  }
  ...
  return (
    <>
      <Surface
        title="Amenities"
        description="Org-curated list of amenities shown on the unit form and inventory filters."
      >
        ...
        <div className="space-y-4">
          ...
          {canWrite && (
            <AddAmenityForm
              ...
              onAdd={(input) =>
                create.mutate(input, {
                  onSuccess: () => toast.success("Amenity created"),
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription render={<div />}>
              <div className="space-y-3">
                {usage.isLoading && <p>Counting affected units…</p>}
                {usage.data && (
                  <>
                    <p>
                      <strong>
                        This amenity is referenced by {usage.data.count} unit
                        {usage.data.count === 1 ? "" : "s"}.
                      </strong>
                    </p>
                    <p>
                      Deleting it will remove the tag from each one. To hide the amenity without
                      losing the tags, deactivate it instead.
                    </p>
                    {usage.data.units.length > 0 && (
                      <div className="max-h-48 overflow-auto rounded border border-[var(--border)] p-2 text-xs">
                        {usage.data.units.map((u) => (
                          <div key={u.id} className="py-0.5">
                            {u.unitCode} — {u.propertyName}
                          </div>
                        ))}
                        {usage.data.count > usage.data.units.length && (
                          <p className="pt-2 text-muted-foreground">
                            Showing first {usage.data.units.length} of {usage.data.count} — total
                            impact: {usage.data.count} units.
                          </p>
                        )}
                      </div>
                    )}
  ...
  return (
    <div className="rounded-lg border border-[var(--card-border)] bg-[var(--page-bg)] p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="text-sm font-semibold text-[var(--text-primary)]">Add new amenity</h4>
      </div>
      <div className="flex flex-wrap items-end gap-3">
        <div className="grid flex-1 gap-1 min-w-[16rem]">
          <span className="text-xs font-medium text-[var(--text-muted)]">Amenity name</span>
          <input
            type="text"
            value={name}
